...
class Settings(BaseSettings):
    ...
    KEYWORD_SCHEMA_TERMS_SCHEMA_DOC_ENABLED: bool = True
    KEYWORD_SCHEMA_TERMS_FEWSHOT_ENABLED: bool = False
    KEYWORD_SCHEMA_TERMS_RAG_CHUNK_ENABLED: bool = False
    KEYWORD_SCHEMA_TERMS_MIN_MATCH_RATIO: float = 0.5
    KEYWORD_SCHEMA_TERMS_CANDIDATE_MULTIPLIER: int = 5
    ...
    SQL_SEMANTIC_VALIDATE_ENABLED: bool = False
    SQL_SEMANTIC_VALIDATE_MODE: str = "warn"  # warn | enforce
    ...
    def _validate_common_settings(self) -> None:
        ...
        if not (0 < self.KEYWORD_SCHEMA_TERMS_MIN_MATCH_RATIO <= 1):
            errors.append("KEYWORD_SCHEMA_TERMS_MIN_MATCH_RATIO 必须在 (0, 1] 范围内")
        if self.KEYWORD_SCHEMA_TERMS_CANDIDATE_MULTIPLIER <= 0:
            errors.append("KEYWORD_SCHEMA_TERMS_CANDIDATE_MULTIPLIER 必须大于 0")
        if self.SQL_SEMANTIC_VALIDATE_MODE not in {"warn", "enforce"}:
            errors.append("SQL_SEMANTIC_VALIDATE_MODE 只能是 warn 或 enforce")
        if errors:
            raise ValueError("; ".join(errors))
