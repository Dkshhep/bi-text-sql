...
链路（融合手绘图① + text2sql 生产要素）：
  ...
  detect_language → rewrite → expand → retrieve_semantics → retrieve ─┬─(无上下文)→ error → END
                                                 └→ schema_linking → generate_sql
       ↑(loop)                                                          ↓
  self_correct ←──(失败且未超次数)── validate_sql → validate_semantics ──(通过)→ execute_sql ─(成功)→ format_answer → END
                                          │                    │
...
def build_graph(checkpointer=None, enable_hil: bool | None = None):
    ...
    g.add_node("retrieve_semantics", nodes.retrieve_semantics)
    ...
    g.add_node("validate_semantics", nodes.validate_semantics)
    ...
    g.add_edge("expand", "retrieve_semantics")
    g.add_edge("retrieve_semantics", "retrieve")
    ...
    g.add_conditional_edges(
        ...
        {"execute_sql": "validate_semantics", "self_correct": "self_correct", "format_answer": "format_answer"},
    )
    g.add_conditional_edges(
        "validate_semantics",
        _route_after_validate,
