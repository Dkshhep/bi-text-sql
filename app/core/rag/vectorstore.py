...
async def upsert_fewshots(rows: list[dict]) -> int:
    ...
    async with pool.connection() as conn, conn.cursor() as cur:
        for r, emb in zip(rows, embeddings, strict=True):
            await cur.execute(
                f"""
                INSERT INTO fewshot_example (db_id, question, sql, embedding, tsv, metadata)
                VALUES (%s, %s, %s, %s, to_tsvector('{JIEBA_CFG}', %s), %s::jsonb)
                """,
                (r["db_id"], r["question"], r["sql"], emb, r["question"], _json(r.get("metadata", {}))),
            )
    return len(rows)


async def delete_context_fewshots(db_id: str) -> int:
    """删除文件式语义层派生的 few-shot，不影响 CSpider 原始样例。"""
    pool = await get_pool()
    async with pool.connection() as conn, conn.cursor() as cur:
        await cur.execute(
            """
            DELETE FROM fewshot_example
            WHERE db_id = %s
              AND metadata->>'source' IN ('manual', 'learned')
            """,
            (db_id,),
        )
        return cur.rowcount or 0
...
async def upsert_semantic_docs(rows: list[dict]) -> int:
    """写入语义层文档。每行含 db_id, doc_type, name, content, payload。"""
    if not rows:
        return 0
    embeddings = await embed_texts([r["content"] for r in rows])
    pool = await get_pool()
    async with pool.connection() as conn, conn.cursor() as cur:
        for r, emb in zip(rows, embeddings, strict=True):
            await cur.execute(
                f"""
                INSERT INTO semantic_doc (db_id, doc_type, name, content, payload, embedding, tsv)
                VALUES (%s, %s, %s, %s, %s::jsonb, %s, to_tsvector('{JIEBA_CFG}', %s))
                """,
                (
                    r["db_id"], r["doc_type"], r["name"], r["content"], _json(r.get("payload", {})),
                    emb, r["content"],
                ),
            )
    return len(rows)
...
async def delete_semantic_docs(db_id: str) -> int:
    """删除某个 db_id 的语义层派生索引。"""
    pool = await get_pool()
    async with pool.connection() as conn, conn.cursor() as cur:
        await cur.execute("DELETE FROM semantic_doc WHERE db_id = %s", (db_id,))
        return cur.rowcount or 0


async def fetch_semantic_docs(db_id: str, doc_types: list[str] | None = None) -> list[dict]:
    """按 db_id 精确读取语义层文档，供全局规则和语义校验使用。"""
    pool = await get_pool()
    async with pool.connection() as conn, conn.cursor() as cur:
        if doc_types:
            await cur.execute(
                "SELECT * FROM semantic_doc WHERE db_id = %s AND doc_type = ANY(%s) ORDER BY id",
                (db_id, doc_types),
            )
        else:
            await cur.execute("SELECT * FROM semantic_doc WHERE db_id = %s ORDER BY id", (db_id,))
