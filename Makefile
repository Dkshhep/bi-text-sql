.PHONY: help install infra-up infra-down migrate ingest ingest-context run smoke test lint fmt check-config rate-limit-test
...
help:
	...
	@echo "ingest-context 编译 context/<db_id> 语义层（DB_ID=concert_singer）"
...
ingest-context:
	python -m eval.ingest_context --db-id $${DB_ID:-concert_singer} --replace
